...
model_df <- merge_df %>% dplyr::select('avg_gpa','male','ethnic','citizenship','restype',
                                       'satR','testscoreR','decileR')
...
### Merge dataframes. 
Check for duplicates in applications and transcripts. For applications there were duplicates for the desired year of admission, so we took the minimum desired year. For transcripts, make sure there are no duplicates for the same semester. We also limited to enrollment at the applied school to track progress only at schools of enrollment.
```{r}
