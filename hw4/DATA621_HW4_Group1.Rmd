...
For example, variable PARENT1 could become binary, and `EDUCATION` could be transformed to `If Higher education` with 1 for education above Bachelor's.
...
Reviewing the top 5 predictor variables that impacted likelihood and the cost associated with an accident:

-   `URBAN1` - working or living in an urban neighborhood increased the expected costs associated with a crash
-   `CAR_TYPE Sports Car` - sports cars increased the expected costs
-   `LIC_REVOKED1` - a history of having a revoked license increased the expected costs
-   `JOB Manager` - being a manager reduced the expected costs
-   `PRIVATE_USE1` - using a car for private activities reduced the expected costs
...
Overall, our approach of subsetting the problem into two separate models seemed to work well.
