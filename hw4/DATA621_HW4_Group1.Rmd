...
The coefficients extracted using lambda.min minimizes the mean cross-validated error. The resulting model incldues 33 non zero coefficients and has an AIC of 60.08.
The coefficients extracted using lambda.1se produce the most regularized model (cross-validated error is within one standard error of the minimum). For this model there are 25 non zero coefficients and it has an AIC of 44.23
...
A closer look at the remaining 37 no zero coefficients for the selected lambda value of lambda.min we can observe the top predictor variables `URBANICITYHighly Urban/ Urban` predictor variable has the largest impact on the response variable `TARGET_AMT`. In the lasso model the coefficient for `URBANICITYHighly Urban/ Urban` home work area is biggest contributor to the cost estimates of a car crash by a factor of 2.

Reviewing the top 5 predictor variables that impact likelyhood and cost associated with an accident:

- `URBANICITYHighly Urban/ Urban` - working or living in an urban neighborhood increase expected cost associated with a crash
- `JOBManager` - being a manager reduces the expected costs associated with a crash
- `CAR_USEPrivate` - using a car for private activities reduces
- `REVOKEDYes` - a history of having a revoked license increases the expected costs associated with a crash
- `JOBDoctor` - being a doctor reduces the expected costs associated with a crash


Some of the notable coefficeints that drop out of the model include:

- `CAR_TYPE` (Truck, Pickup, Van or Red)
- Sex of the driver
- `EDUCATION` (PhD)
...
lassoPred <- predict(lasso.lm.model, newx = X_test, type = "response", s = 'lambda.1se')
...
Analysing a scatter plot of the prediction errors vs measured costs and a comparative histogram of the predicted and measured costs highlights a shortcomings of the lasso model.The model consistently predicts crash costs lower than the actual measured crash costs. The gap is more pronounced when looking at predicted and 
...
The Standardized Residuals plot shows increasing variance at hight values of the response variable.
...
The cv.glmnet() function was also used as logistic regression model. Similar to the regression model k-fold cross-validation was perfromed with variable selection using lasso regularization. The following attribute settings were selected for the model:
...
- alpha =1 - The alpha value of 1 sets the variable shrinkage method to lasso. 
- weights = a weight of 0.2638 / n for observation with a 0 `TARGET_FLAG` and 0.7362 / n obervations with a 1 value of `TARGET_FLAG`.
- standardize = TRUE - Finally, we explicitly set the standardization attribute to TRUE; this will normalize the prediction variables around a mean of zero and a standard deviation of one before modeling. 
...
A closer look at the remaining 36 no zero coefficients for the selected lambda value of lambda.min we can observe the `URBANICITYHighly Urban/ Urban` predictor variable has the largest impact on the prediction of a car crash by a factor of three. Reviewing the top 5 predictor variables that impact likelihood and cost associated with an accident:
...
The coefficients extracted at the lambda.min value are used to predict the lickihood of an accident. The confusion matrix highlights an accuracy of 73.7%.
...
Again we check linear relationship between independent variables and the Logit of the target variable. Visually inspecting the results there is a linear trend in the relationship but there are deviations from the straight line in all variables.
