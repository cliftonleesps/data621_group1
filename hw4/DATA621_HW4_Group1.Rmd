...
train_transformed$TARGET_AMT <- log(train_transformed$TARGET_AMT + 1)
...
results_log <- rbind(results_log,tibble(Model = "Model #3: Lasso", Accuracy="", 
...
```{r model_1}
 df_train <- linear_train_data%>% dplyr::select(-c(TARGET_FLAG))
df_test <- linear_valid_data%>% dplyr::select(-c(TARGET_FLAG))
# build model
...
model_1_aic.eval <- bind_cols(target = df_test$TARGET_AMT, predicted=model_1_aic.valid)
...
```{r}
# validate and calculate RMSE
stepwise.lm.valid <- predict(model_2_aic, newdata = amt_test)
stepwise.lm.eval <- bind_cols(target = amt_test$TARGET_AMT, predicted=stepwise.lm.valid)
...
t_df <- linear_train_data %>% dplyr::select(-c(TARGET_FLAG))
...
w <- table(linear_train_data$TARGET_FLAG)
n <- length(Y)
prop_0 <- w[1] / (w[1] + w[2])
prop_1 <- w[2] / (w[1] + w[2])

# calculate the weights vector
#weights_lm  <- ifelse(linear_train_data$TARGET_FLAG == 0,  prop_1/n, prop_0/n)
```
...
```{r fig.show="hold", out.width="50%", fig.height=6, eval=FALSE}
...
```{r eval=FALSE}
t0_df <- linear_valid_data %>% dplyr::select(-c(TARGET_FLAG)) 
...
pred_ln_df <- linear_valid_data %>% drop_na()
...
# build X matrix and Y vector
t0_df <- linear_valid_data %>% dplyr::select(-c(TARGET_FLAG)) %>% drop_na()
X_test <- model.matrix(TARGET_AMT ~ ., data=t0_df)[,-1]
Y_test <- t0_df[,"TARGET_AMT"] 
