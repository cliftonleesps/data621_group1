...
lasso.ln.model<- cv.glmnet(x=X,y=Y,
                       family = "gaussian",
                       #link = "logit",
...
l.min <- lasso.ln.model$lambda.min
l.1se <- lasso.ln.model$lambda.1se
coef(lasso.ln.model, s = "lambda.min" )
coef(lasso.ln.model, s = "lambda.1se" )
lasso.ln.model
...
plot(lasso.ln.model)
plot(lasso.ln.model$glmnet.fit, xvar="lambda", label=TRUE)
plot(lasso.ln.model$glmnet.fit, xvar='dev', label=TRUE)
...
```{r}
assess.glmnet(lasso.ln.model,           
              newx = X,              
              newy = Y )    

print(glmnet_cv_aicc(lasso.ln.model, 'lambda.min'))
print(glmnet_cv_aicc(lasso.ln.model, 'lambda.1se'))
...
```{r}
coef(lasso.ln.model, s = "lambda.min" )
vip(lasso.ln.model, num_features=20 ,geom = "col", include_type=TRUE, lambda = "lambda.min")
```
...
# 
t0_df <- test_data %>% dplyr::select(-c(TARGET_FLAG)) %>% drop_na()
...
lassoPred <- predict(lasso.ln.model, newx = X_test, type = "response", s = 'lambda.min')

pred_ln_df <- test_data %>% drop_na()
...
# 
t0_df <- test_data %>% dplyr::select(-c(TARGET_AMT)) %>% drop_na()
