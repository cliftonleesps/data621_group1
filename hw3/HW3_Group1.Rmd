...
As expected from the confusion matrices, we see the ROC curves for model 1 and 2 being closer to ideal. 
...
Even though model 3's accuracy is the lowest, its AIC is the lowest so we'll choose it for predictions.
...
**ONE OF THE DATA BELOW, DEPENDS ON WHAT MODEL WE USE**

For the first model, we will transform variables `chas` and `rad` to categorical. Also, we remove `zn` column as it had 0 value as the most occurred.
```{r factor_eval}
