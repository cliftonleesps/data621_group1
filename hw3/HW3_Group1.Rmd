...
- The coefficients extracted using lambda.min minimizes the mean cross-validated error. The resulting model drops one predictor variable rm and has an AIC of -352.6. 
- The coefficients extracted using lambda.1se produce the most regularized model (cross-validated error is within one standard error of the minimum). For this model the 'rm' and 'tax' predictor variables dropout, generating an AIC of -334.3.
...
Taking a closer look at the coefficients for the selected lamdba values  

```{r}
coef(lasso.model, s = "lambda.min" )
vip(lasso.model, geom = "point")
```
...
lassoPred <- predict(lasso.model, newx = X_new, type = "response", s = 'lambda.min')
...
Again we check linear relationship between independent variables and the Logit of the target variable. Visually inspecting the results only `dis`, `indus`, `nox`, `rm` and `tax` seem like linear relation with the logit results.
...
par(mfrow=c(2,2))
rocs <- roc.glmnet(lasso.model, newx = X_new, newy = Y_new )
plot(rocs,type="l")  
```
...
lasso.r2 <- glmnet_cv_aicc(lasso.model, 'lambda.min')
