...
In terms of multicollinearity, all variables have vif less than 5. As a result, multicollinearity shouldn't be a problem for our model.
```{r model_1_vif}
car::vif(model_1_aic)
```
...
The stepAIC function tells us to include `nox`, `rm`, `age`, `dis`, `rad` and `medv` variables.
...
### 4.1 Confusion matrix, ROC curve
To measure model performance, a confusion matrix (Classification Error Rate, Precision, Sensitivity, Specificity, F1 Score) and ROC curve will be used.
...
par(mfrow=c(2,2))
roc(transformed_testing$target, transformed_testing$model_1, plot = TRUE,  print.auc = TRUE, main="Model 1 - ROC Curve")

roc(box_testing$target, box_testing$model_2, plot = TRUE,  print.auc = TRUE, main="Model 2 - ROC Curve")
```
