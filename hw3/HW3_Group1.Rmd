...
To build our model, we need to transform categorical variables `chas`, `target` and `rad` from numeric (default by the data) to factors. `rad` in particular is an ordinal categorical index. The data will be split for training and testing to train and check our models.
As we saw from the summary tables above, there are `no missing values` to deal with.
...
These results seem strange as higher median value of owner-occupied homes in 1000s dollars (`medv`) should lead to lower crime rate. The same would be for `age`. It may be a results of multicollinearity.
...
Only `nox` and `dis` have regression coefficients in model that are significant. The coefficients of `nox` (nitrogen oxides concentration), `medv` (median value of owner-occupied homes in 1000s dollars), `age` (proportion of owner-occupied units built prior to 1940), `dis` (weighted mean of distances to five Boston employment centers) increase the chance of crime.
The results are strange again as in the model 1. A higher median value of owner-occupied homes in 1000s dollars (`medv`) should lead to lower crime rate. The same would be for `age`. It may be a results of multicollinearity.
...
Even though model 3's accuracy is the lower than for model 1, its AIC is the lowest as well as Deviance, so we'll choose it for predictions. Based on the results of  sensitivity, specificity, AUC we suspect the over-fitting problem with models 1 and 2. Models 1 and 2 are similar as we used stepAIC for both of them when selecting variables but it happened that BoxCox transformations lower AIC result for model 2.
...
```{r roc_model_2, warning=FALSE, message=FALSE}
par(mfrow=c(2,2))
...
We are going to follow transformations that we did for the training data using Lasso functions. The results of the prediction will be written to the `eval_predictions_lasso.csv` and loaded to the [GitHub repo](https://github.com/ex-pr/data621_group1/tree/main/hw3).
...
## 5. Conclusion
  ...
  After investigating a few models, the third model where k-fold cross-validation with variable selection using lasso regularization performed the best with an AIC score of -337 when incorporating eight predictors, out of a possible twelve. The model achieved an accuracy rate of 94%. With this and similar models, city councils can devote resources and attention to neighborhoods with a higher crime risk as well as better plan future development.
