...
describe(train_df) %>%  kable("html") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F) %>% scroll_box(width = "100%")
...
log_train_df$zn <- log(log_train_df$zn+1)
...
box_train_df$nox <- BoxCox(transformed_train_df$nox, age_lambda)
...
```{r box_hists}
m_df_box <- box_train_df %>% select(c(age, dis, lstat, nox, ptratio)) %>% melt() 

a <- m_df_box %>% ggplot(aes(x= value)) + 
geom_density(color='#023020', fill='gray') + facet_wrap(~variable, scales = 'free',  ncol = 1) + theme_bw()

m_df <- transformed_train_df %>% select(c(age, dis, lstat, nox, ptratio)) %>% melt() 
...
**FROM DAVID LASSO. PLEASE ADD TEXT**
...
trainining_df  <- subset(train_df, sample == TRUE)
testing_df   <- subset(train_df, sample == FALSE)
