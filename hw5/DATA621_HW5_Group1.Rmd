...
```{r}
# load data
df_train <- as.data.frame(read.csv('data/wine-training-data.csv'))
df_predict <- as.data.frame(read.csv('data/wine-evaluation-data.csv'))
...
-   The 'evaluation' dataset contains no values for **TARGET**, suggesting this data might be used for prediction rather than validation and evaluation of model performance.
    For clarity we'll rename this this dataset 'prediction' instead and create a separate validation hold-out from the training data.
...
-   The training dataset consists of `r nrow(df_train)` observations containing `r ncol(df_train)-1` predictor variables.
...
First, can safely assume that all wines in this dataset had an actual `ph` score greater than zero, which is the most acidic rank, on the scale of powerful industrial acids.
We'll want to impute reasonable values for these.

Based on some reading into the organic wines segment, where there is a growing demand in the market for specialty products such as low-sulfite, low-sugar and low-alcohol wines.
