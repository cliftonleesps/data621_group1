...
```
The histogram of the target variable showed normal distribution though it seemed zero inflated. It was considered when building some of the models.
```{r}
df_all%>%
  drop_na(target) %>%
  ggplot() +
...
In the correlation plot below, we observed that variables `Star` and `LabelAppeal` were the most positively correlated to the response variable (more stars, as a result, more purchases/ better label, more purchases).
There were a slight negative correlation between `AcidIndex` and the `target` variable.
...
The field `index` was removed from the data as it didn't affect anything in the models and just meant the index of an observation.
...
A number of numerical characteristics exhibited negative values that seem somewhat implausible, however, we had decided to trust that these variables had undergone log transformation and that the values were valid.

### 2.2 Missing Data

Next we decided find and impute any missing data.
...
Heeding the warning in the assignment, *"sometimes, the fact that a variable is missing is actually predictive of the target"*, we considere each of these variables carefully.
While there could be data "missing completely at random" (MCAR) that we wished to impute, this may not always be the case.
...
#added 0 instead of NA later in the code
```
...
First, we safely assumed that all wines in this dataset had an actual `ph` score greater than zero (which would represent the most acidic rank, such as powerful industrial acids). We decided to impute more reasonable values for these.

Based on some reading into the organic wines segment, there were a growing demand in the market for specialty products such as low-sulfite, low-sugar and low-alcohol wines.
However, this still represented a very small segment of the overall market, and chemically it was not likely for these compounds to be completely absent from the final product.

Additionally, the predictors `freesulfurdioxide` and `totalsulfurdioxide` were linked - the amount of 'Free' SO2 in wine was always a subset of the 'Total' S02 presented.
...
This could be candidate for handling with Zero-Inflated models.
...
After the previous operations, we had reasonably imputed values, and nearly-normal distributions for our numeric predictors, taking special note of the frequency of zero values for `labelappeal` and `stars`.
...
With transformations complete, we splited back into training and prediction datasets based on our `source_flag`, and created a 15% validation hold-out from the training data.
...
The data used for the models has no missing values (stars=NA was substituted with stars=0), `Label Appeal` and `Acid Index` variables were transformed to factors. 
...
We noted that our model has generated 'dummies' from our categorical variables `labelappeal` and `stars`, and of the 14 total predictors, all but 5 had statistical significance.
...
The `residual deviance` of `r round(pr1_aic$deviance,2)` defined how well the target variable can be predicted by the AIC model that we fit with the predictor variables listed above.
...
The resulting plot 1 below (predicted values vs. residuals) showed that a lot of the standardized residuals fell outside the range of -2 and 2. Examining the Standardized Pearson Residuals plot, the model seemed good until the middle range but there were extremes on the right.
In general, this model didn't fit our data as there were zero counts in the response variable. We had to consider models that could address this issue.
...
Sometimes a count response data has the number of zeroes appearing is significantly greater than the Poisson or negative binomial models would predict. As a result, we built a Zero-Inflated Poisson model to handle the large number of zero values in our `labelappeal`, `stars` predictors and `target` response, to see if we can improve model accuracy. 
...
The results for the positive/negative coefficients seemed strange in the zero-inflation model. We should remember that our zero-inflated model produced two sets of coefficients — one for the truncated Poisson part of the model and one for the binary response part of the model. The count coefficients estimated the `target`>0 (number of purchases): good label and high rating increased purchases (the same as in the regular poisson model), high acid index decreased it. The zero-inflation model coefficients estimated the probability of whether or not any purchases was made: no reviews increased the chance of no-purchase.


The `residual deviance` was `r #round(pr2$deviance,2)`
...
Notably, our Dispersion Parameter was `r #round(pr2['Dispersion'],2)`, which suggested a degree of under-dispersion in the data.
...
The resulting plot below (predicted values vs. residuals) showed that a lot of the standardized residuals fell outside the range of -2 and 2.
...
To use the Poisson models, the assumption that mean and variance of the target variables were equal should  have been observed. Though, we saw the violation of the assumption. As a result, Negative Binomial distribution was more appropriate as it required the variance to be larger than the mean.
...
Generally, we would use Negative Binomial Regression in cases of over-dispersion (where the variance of our dependent variable is significantly greater than the mean). This does not appear to be the case with our dataset, there were a small difference between the mean and the variance, so we applied it here and examined the results:
...
The `residual deviance` was `r round(nb1$deviance,2)`
Though residual deviance was greater than the degrees of freedom that meant over-dispersion (the estimates were correct, but the standard errors were wrong and unaccounted for by the model).
...
Theta parameter seemed too large (40419). This occurred because the data is almost equidispersed (variance == mean), which couldn't be achieved by a negative binomial distribution. As a result, the optimizer gave up while it was trying to send the dispersion parameter to infinity.

As expected, the Negative Binomial Regression did not outperform the Poisson. There were little difference between poisson and negative binomial models due to the little difference between mean and variance.
...
The resulting plot 1 below (predicted values vs. residuals) showed that a lot of the standardized residuals fell outside the range of -2 and 2. Examining the Standardized Pearson Residuals plot, the model seemed good only the middle range but there were extremes on the ends.
In general, this model didn't fit our data as there were zero counts in the response variable. We had to consider models that could address this issue.
...
The results for the positive/negative coefficients seemed reasonable and similar to the poisson models. The count coefficients estimated the `target`>0 (number of purchases): good label and high rating increased purchases (the same as in the regular poisson model), high acid index decreased it. The zero-inflation model coefficients estimated the probability of whether or not any purchases was made: no reviews increased the chance of no-purchase. 

The `residual deviance` was `r #round(nb2$deviance,2)`
...
The resulting plot below (predicted values vs. residuals) showed that a lot of the standardized residuals fell outside the range of -2 and 2 but it seemed better than for the Poisson models.
...
Results suggested that the performance improved after using zero-inflated models.
...
-   AIC/BIC was lowest for the Poisson model 2, the worst results were for Poisson/Negative Binomial models without modeling an excess of zero counts.
-   MAE (mean absolute error between observed and predicted values) was the lowest for the Poisson model 2, Multiple linear models performed with higher mean error.
...
Overall, Zero-inflated Poisson model appeared to produce the best fit.
...
Using Zero-Inflated Poisson regression model (no missing values, some variables to factors), we predicted `target` values for the `r sum(try_df_predict$target_predict)` observations in our evaluation dataset.
...
The most important predictive factors for the number of purchases - by far - were the amount of stars (1, 2, 3, 4 or none), marketing score indicating the appeal of label design for consumers (LabelAppeal), and  total acidity of wine (AcidIndex). As a result, based on our models, a good wine should have good reviews, low acid index, and attractive label. These results were not far from reality since majority of the people wouldn't care about citric acid content
fixed acidity, volatile acid content, ph, etc.

There were some issue with the accuracy of predictions for the 0 number of cases purchased (target=0). This could be a candidate for further research and analysis together with the high theta being produced for the Negative Binomial models.

For future consideration, we might also wish to look into advanced variable selection, it could improve the performance of our models.
