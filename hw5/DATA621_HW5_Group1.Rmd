...
These variables included  **ADD TEXT**
...
-   4 variables are discrete (`stars`, `labelappeal`, `acidindex`, `target`)
-   8 variables out of 14 contain missing values
-   `target` (number of cases purchased) varies between 0 and 8

```{r data_load}
# load data
df_trains <- read.csv('https://raw.githubusercontent.com/cliftonleesps/data621_group1/main/hw5/data/wine-training-data.csv')
...
df_trains<- df_trains %>% rename(INDEX=п.їINDEX)

# union for exploration and prep
df_trains <- df_trains %>% mutate(source='train')
df_predict <- df_predict %>% mutate(source='predict')
df_all <- bind_rows(df_trains, df_predict)
...
Variables `acidindex` (proprietary method of testing total acidity of wine by using a weighted average) seem to be slightly right-skewed.
...
We also see the relationship between `stars`/`labelappeal`and `target`  variables. If `labelappeal` increases, `target` variable also goes up (if score indicating the appeal of label design for consumers increases, number of cases purchased increases as well). `stars=NA`correlates to low values for `target`. We may use the hint from from the assignment that a variable is missing is actually predictive of the target, and won't get rid of the missing values for `stars`.
...
In the correlation plot below, we see that variables `Star` and `LabelAppeal` are the most positively correlated to the response variable (more stars, as a result, more purchases/ better label, more purchases). There is slight negative correlation between `AcidIndex` and the `target` variable.
...
If we assume the existence of a review has some impact on the sales of a wine brand (whatever the reviewer's sentiment), then imputing mean or predicted values here might distort our model. Therefore, we'll simply preserve the NAs as the model functions automatically adjust for these observations.
...
```{r, echo=TRUE}
pr_try <- glm(target ~ ., family = 'poisson', data = try_train)
```

```{r}
summary(pr_try)

pr_try_diag <- model_diag(pr_try)

pr_try_diag[c('AIC','Dispersion','Log-Lik')] %>% 
  round(2) %>% nice_table()
```
...
coef(fit.lasso, minAic)
cat("theta estimate", fit.lasso$theta[minAic])
```
...
The coefficients for the zero inflation model that survice the regularization process include `stars`, `labelappeal-1`, `labelappeal1` ,and `volatileacidity`.
...
```{r}
lasso_eval <- predict(fit.lasso, newdata=df_valid, which=minAic)
lasso_eval <- data.frame(lasso_eval)
colnames(lasso_eval) <- c('predicted')
lasso_eval$target <- df_valid$target
