...
# Create boolean for presence of review ..removed, linearly related in model
# df_prep <- df_prep %>%
...
In our training and validation dataset, the mean is `r round(mean(df_train_all$target),2)` and the variance is `r round(var(df_train_all$target),2)`, suggesting our data is roughly equally dispersed.
...
```{r}
# pearson resids, dispersion parameter, log-likelihood, AIC
pr1_pres <- resid(pr1, 'pearson')
pr1_disp <- sum(pr1_pres^2)/pr1$df.residual
pr1_loglik <- logLik(pr1)
pr1_aic <- AIC(pr1)
```

### Diagnostics

While our Dispersion Parameter is less than one (`r round(pr1_disp,3)`), by graphing our target values (green) against our predicted values (blue) we can easily see this model tends to under-predict the higher count levels, and over-predict the lower count levels.

```{r}
# table
pr1_diag_tbl <- matrix(c(round(pr1_aic,3), round(pr1_disp,3), round(pr1_loglik,3)))
rownames(pr1_diag_tbl) <- c('AIC','Dispersion','LogLik')
pr1_diag_tbl %>% kable %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                html_font = 'monospace',
                full_width = FALSE)
...
```{r}
# pearson resids, dispersion parameter, log-likelihood, AIC
pr2_pres <- resid(pr2, 'pearson')
pr2_disp <- sum(pr2_pres^2)/pr1$df.residual
pr2_loglik <- logLik(pr2)
pr2_aic <- AIC(pr2)
```
...
```{r}
# table
pr2_diag_tbl <- matrix(c(round(pr2_aic,3), round(pr2_disp,3), round(pr2_loglik,3)))
rownames(pr2_diag_tbl) <- c('AIC','Dispersion','LogLik')
...
```{r}
# pearson resids, dispersion parameter, log-likelihood, AIC
nb1_pres <- resid(nb1, 'pearson')
nb1_disp <- sum(nb1_pres^2)/nb1$df.residual
nb1_loglik <- logLik(nb1)
nb1_aic <- AIC(nb1)
```
...
```{r}
# table
nb1_diag_tbl <- matrix(c(round(nb1_aic,3), round(nb1_disp,3), round(nb1_loglik,3)))
rownames(nb1_diag_tbl) <- c('AIC','Dispersion','LogLik')
...
```{r}
# pearson resids, dispersion parameter, log-likelihood, AIC
nb2_pres <- resid(nb2, 'pearson')
nb2_disp <- sum(nb2_pres^2)/pr1$df.residual
nb2_loglik <- logLik(nb2)
nb2_aic <- AIC(nb2)
```
...
```{r}
# table
nb2_diag_tbl <- matrix(c(round(nb2_aic,3), round(nb2_disp,3), round(nb2_loglik,3)))
rownames(nb2_diag_tbl) <- c('AIC','Dispersion','LogLik')
