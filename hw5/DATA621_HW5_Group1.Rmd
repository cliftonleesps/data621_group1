...
a <- coef(fit.lasso, minBic)
round(data.frame(a$count),4)
round(data.frame(a$zero),4)
...
lasso_eval <- predict(fit.lasso, newdata=df_valid, which=minBic)
...
results_lm_tbl <- results_lm_tbl %>% add_row(tibble_row(
                      ...
                      mpe = m_df[[4,3]],
                      "RMSE" = m_df[[5,3]],
                      AIC = as.numeric(AIC(fit.lasso)[minBic]),
                      "Adjusted R2" = NA,
                      "F-statistic" = NA
                     ))
```
