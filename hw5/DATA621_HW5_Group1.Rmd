...
The histogram of the target variable showed normal distribution though it seemed zero inflated.
It was considered when building some of the models.
...
First, we safely assumed that all wines in this dataset had an actual `ph` score greater than zero (which would represent the most acidic rank, such as powerful industrial acids).
We decided to impute more reasonable values for these.
...
As discussed before, high rating and good labels lead to better sales:
...
The results followed the common logic, the higher total acidity of wine and no rating lead to low sales:
...
The resulting plot 1 below (predicted values vs. residuals) showed that a lot of the standardized residuals fell outside the range of -2 and 2.
...
Sometimes a count response data has the number of zeroes appearing is significantly greater than the Poisson or negative binomial models would predict.
As a result, we built a Zero-Inflated Poisson model to handle the large number of zero values in our `labelappeal`, `stars` predictors and `target` response, to see if we can improve model accuracy.
...
We should remember that our zero-inflated model produced two sets of coefficients --- one for the truncated Poisson part of the model and one for the binary response part of the model.
The count coefficients estimated the `target`\>0 (number of purchases): good label and high rating increased purchases (the same as in the regular poisson model), high acid index decreased it.
The zero-inflation model coefficients estimated the probability of whether or not any purchases was made: no reviews increased the chance of no-purchase.
...
To use the Poisson models, the assumption that mean and variance of the target variables were equal should have been observed.
Though, we saw the violation of the assumption.
As a result, Negative Binomial distribution was more appropriate as it required the variance to be larger than the mean.
...
This does not appear to be the case with our dataset, there were a small difference between the mean and the variance, so we applied it here and examined the results:
...
Theta parameter seemed too large (40419).
This occurred because the data is almost equidispersed (variance == mean), which couldn't be achieved by a negative binomial distribution.
...
There were little difference between poisson and negative binomial models due to the little difference between mean and variance.
...
The resulting plot 1 below (predicted values vs. residuals) showed that a lot of the standardized residuals fell outside the range of -2 and 2.
Examining the Standardized Pearson Residuals plot, the model seemed good only the middle range but there were extremes on the ends.
In general, this model didn't fit our data as there were zero counts in the response variable.
We had to consider models that could address this issue.
...
The most important predictive factors for the number of purchases - by far - were the amount of stars (1, 2, 3, 4 or none), marketing score indicating the appeal of label design for consumers (LabelAppeal), and total acidity of wine (AcidIndex).
As a result, based on our models, a good wine should have good reviews, low acid index, and attractive label.
These results were not far from reality since majority of the people wouldn't care about citric acid content fixed acidity, volatile acid content, ph, etc.

There were some issue with the accuracy of predictions for the 0 number of cases purchased (target=0).
