...
```
 ...
 [1] "TARGET_WINS"      "TEAM_BATTING_H"   "TEAM_BATTING_2B"  "TEAM_BATTING_3B"  "TEAM_BATTING_HR"  "TEAM_BATTING_BB" 
 [7] "TEAM_BATTING_SO"  "TEAM_BASERUN_SB"  "TEAM_BASERUN_CS"  "TEAM_PITCHING_H"  "TEAM_PITCHING_HR" "TEAM_PITCHING_BB"
[13] "TEAM_PITCHING_SO" "TEAM_FIELDING_E"  "TEAM_FIELDING_DP" "TEAM_BATTING_1B"  "na_flag"    
...
#columnNames <- c('TARGET_WINS')
df <- filterOutliers(filter_df, droppedDf, columnNames)
...
cNames <- names(droppedDf) - c("TEAM_BATTING_SO")
cNames[!cNames %in% c("TEAM_BATTING_SO","na_flag")]




df1 <- droppedDf[filterFlag,]


no_outliers <- subset(data, data$Apperance > (Q1 - 1.5*IQR) & data$Apperance < (Q3 + 1.5*IQR))

no_outliers <- subset(data, !(abs(TEAM_BATTING_H - median(TEAM_BATTING_H)) > 1.5*IQR(TEAM_BATTING_H)))
...
cNames <-  c("TEAM_BATTING_H","TEAM_BATTING_2B")

df1 <- droppedDf
df1 <- remove_sd_outlier(df1, cols = cNames , n_sigmas = 3, verbose = TRUE)
...
aicPred <- lm3step %>% predict(evalImputeDf)
