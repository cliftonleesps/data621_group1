...
lm3step.final <- update(lm3step, . ~ . -TEAM_PITCHING_BB -TEAM_BATTING_BB -TEAM_BASERUN_CS -TEAM_BATTING_2B)
#tab_model(lm3step.final, show.df = TRUE, show.aic = TRUE, show.fstat=TRUE, show.se = FALSE, digits.p=4)
...
vif_values %>%
  ...
  geom_hline(yintercept=5, linetype="dashed", color = "red") +
  geom_bar(stat = 'identity', width=0.3 ,position=position_dodge()) +
  labs(title = 'Model 4: Variance Inflation Factor')
...
tab_model(lm3, lm3step, lm3step.final, 
          ...
          show.df = FALSE, show.aic = TRUE, show.fstat=TRUE, show.se = FALSE, digits.p=4)
