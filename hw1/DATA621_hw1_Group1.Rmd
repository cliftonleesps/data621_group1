...
author: 'Critical Thinking Group 1: Ben Inbar, Cliff Lee, Daria Dubovskaia, David
  Simbandumwe, Jeff Parks, Nick Oliver'
output:
  html_document:
    toc: yes
    toc_float: yes
...
m_df <- trainDf %>% dplyr::select(-c(INDEX))
...
droppedDf <- filterOutliers(filterDf, droppedDf, columnNames)
...
transformed_df <- transformed_df |> subset(TEAM_PITCHING_H < 5000, TEAM_PITCHING_BB < 1100) #TEAM_PITCHING_BB < 1100, TEAM_PITCHING_SO < 2500, TEAM_FIELDING_E < 500)
...
The third model will contain the variables from the cleaned dataset with outliers omitted using historic data from the Lahman's Baseball Database. In baseballs early history very few games were played thus the calculation used to normalize the statistics to a 162 game season creates outliers. The min and max from the modern era (post 1900) are used to filter outliers.
...
The diagnostic plots show residuals that are normally distributed with a linear relationship to the fitted values. The homogeneity of variance plot is curved, indicating that the variance of residuals is not consistent. 
...
```{r echo=FALSE}
lm4 <- lm(TARGET_WINS~., data=transformed_df)
...
```{r echo=FALSE}
r <- c(lm1Sum$r.squared, lm2Sum$r.squared, lm3sum$r.squared)
rsse <- c(lm1Sum$sigma, lm2Sum$sigma, lm3sum$sigma)
adjusted.r <- c(lm1Sum$adj.r.squared, lm2Sum$adj.r.squared, lm3sum$adj.r.squared)
...
evalDroppedDf <- evalImputeDf %>% mutate(TEAM_BATTING_1B = TEAM_BATTING_H - TEAM_BATTING_2B - TEAM_BATTING_3B - TEAM_BATTING_HR)

transformed_eval <- evalImputeDf |> mutate_each(funs(sqrt),
                   batting_hr_sqrt = TEAM_BATTING_HR,
                   batting_so_sqrt = TEAM_BATTING_SO,
                   baserun_cs_sqrt = TEAM_BASERUN_CS,
                   pitching_hr_sqrt = TEAM_PITCHING_HR)
...
plot(1:nrow(xT), xT$aic4, xlab="AIC4", ylab="Wins", title="AIC4")
```
...
```{r echo=FALSE}
check_model(lm3step, check = c("vif","normality","linearity","homogeneity"))
```

**Model 4**

```{r echo=FALSE}
check_model(lm4step, check = c("vif","normality","linearity","homogeneity"))
```
