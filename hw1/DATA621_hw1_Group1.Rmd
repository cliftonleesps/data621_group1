...
Now we will build four separate linear models and compare their performance and fit. 
...
For the second model we will use the cleaned data set with missing values imputed using the MICE method. In addition I will try to select variables which result in a better fit using my out intuition. 
...
aicLm4 <- stepAIC(lm4, trace=FALSE)
aicSum4 <- summary(aicLm4)
...
```{r}
plot(compare_performance(lm1,lm2,aicLm,aicLm4, rank=T))
```
...
aicPred4 <- aicLm4 %>% predict(evalImputeDf)
```
...
```{r}
check_model(aicLm4, check = c("vif","normality","linearity","homogeneity"))
```
## Conclusion
